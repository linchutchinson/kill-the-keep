...
pub fn select_card_targets(
    ...
    if is_mouse_button_released(MouseButton::Left) {
        ...
        if let Ok(_targeted_card) = ecs
            .entry_ref(*card_entity)
            .unwrap()
            .get_component::<SelectTarget>()
        {
            ...
            targets_query
                ...
                .for_each(|(entity, pos, _, sprite)| {
                    let tl = Vec2::new(
                        pos.x - sprite.texture.width() * 0.5,
...
pub fn play_card(
    ...
    if let Ok(energy_cost) = ecs
        .entry_ref(message.card)
        .unwrap()
